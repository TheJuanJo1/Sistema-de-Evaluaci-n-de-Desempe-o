                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end space-x-2">
                                    @role('Administrador|Talento Humano|Rector')
                                        @if(isset($worker->model_type) && $worker->model_type === 'worker')
                                            @php $editRoute = route('workers.edit', $worker->id); @endphp
                                            <a href="{{ $editRoute }}" class="inline-flex items-center p-2 text-slate-600 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition duration-200" title="Editar">
