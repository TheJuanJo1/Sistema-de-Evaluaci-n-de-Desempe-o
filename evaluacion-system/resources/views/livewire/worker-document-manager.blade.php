    ...
    <div class="bg-slate-50 rounded-2xl p-6 border border-slate-200 border-dashed" x-data="{
        ...
        validateFile(event) {
            ...
            if (file.size > 5242880) {
                this.clientError = 'El archivo supera el límite permitido de 5 MB. Selecciona otro archivo.';
                input.value = ''; // Limpiar el input
                @this.set('file', null); // Limpiar en Livewire
                return;
            }
            ...
            if (!allowedExtensions.includes(extension)) {
                this.clientError = 'Formato no permitido. Solo se aceptan archivos PDF, JPG, JPEG y PNG.';
                input.value = ''; // Limpiar el input
                @this.set('file', null); // Limpiar en Livewire
                return;
            }
        ...
        <form wire:submit.prevent="upload" enctype="multipart/form-data" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label for="documentName" class="block text-sm font-semibold text-slate-700 mb-1">Nombre del Documento</label>
                ...
                <div>
                    <label for="documentType" class="block text-sm font-semibold text-slate-700 mb-1">Tipo de Documento</label>
            ...
            <div class="mt-4">
                <label for="file" class="block text-sm font-semibold text-slate-700 mb-1">Archivo (PDF, JPG, PNG - Máx 5MB)</label>
                <div class="flex items-center justify-center w-full">
                    <label class="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-300 border-dashed rounded-xl cursor-pointer bg-white hover:bg-slate-50 transition duration-200">
                        <div class="flex flex-col items-center justify-center pt-5 pb-6">
                            ...
                            <p class="mb-2 text-sm text-slate-500"><span class="font-semibold text-blue-600">Haz clic para buscar</span> o arrastra y suelta</p>
                        </div>
                        <input id="file" wire:model="file" type="file" class="hidden" @change="validateFile($event)"/>
                    </label>
                </div>
                
                <!-- Client side validation error -->
                <template x-if="clientError">
                    <div class="mt-2 text-xs font-semibold text-red-600 flex items-center">
                ...
                <div wire:loading wire:target="file" class="mt-2 text-xs text-blue-600 font-semibold flex items-center">
                    <svg class="animate-spin -ml-1 mr-2 h-4 w-4 text-blue-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path>
                    </svg>
                    Procesando y cargando archivo en servidor...
                </div>
            ...
            @if ($file && !$errors->has('file'))
                <div class="mt-4 p-4 bg-white border border-slate-200 rounded-xl flex items-center space-x-4 shadow-sm animate-fade-in">
                    <!-- Preview Thumbnail or Icon -->
                    <div class="relative flex-shrink-0 h-16 w-16 rounded-lg border border-slate-100 bg-slate-50 overflow-hidden flex items-center justify-center">
                        ...
                        @else
                            <div class="flex flex-col items-center justify-center text-red-500">
                                ...
                                <span class="text-[9px] font-black uppercase mt-0.5">{{ $file->getClientOriginalExtension() }}</span>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-bold text-slate-800 truncate" title="{{ $file->getClientOriginalName() }}">
                            {{ $file->getClientOriginalName() }}
                        </p>
                        <p class="text-xs text-slate-500 font-medium">
                            Tamaño: {{ round($file->getSize() / 1024, 1) }} KB
                        </p>
                    </div>

                    <!-- Cancel / Remove -->
                    <button type="button" wire:click="$set('file', null)" class="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition duration-200" title="Quitar archivo">
